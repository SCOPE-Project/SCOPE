...
def kep2rv(kep: ArrayLike) -> NDArray[np.float64]:
    ...
    if kep.ndim != 2 or kep.shape[1] != 6:
        raise ValueError(
            f"kep must have shape (m, 6) or (6,), but received {kep.shape}."
        )
    ...
    if np.any(p <= 0.0):
        raise ValueError("The semi-latus rectum must be positive. Check a and e.")
    ...
    rotation[:, 0, 0] = cos_raan * cos_omega - sin_raan * sin_omega * cos_i
    rotation[:, 0, 1] = -cos_raan * sin_omega - sin_raan * cos_omega * cos_i
    rotation[:, 0, 2] = sin_raan * sin_i

    rotation[:, 1, 0] = sin_raan * cos_omega + cos_raan * sin_omega * cos_i
    rotation[:, 1, 1] = -sin_raan * sin_omega + cos_raan * cos_omega * cos_i
