...
class ActivityBlueprint(BaseModel):
    ...
    creation_time: AwareDatetime = Field(
        default=datetime.now().astimezone(tz=UTC), description="Creation time of the activity blueprint."
    )
    edit_time: AwareDatetime = Field(
        default=datetime.now().astimezone(tz=UTC), description="Edit time of the activity blueprint."
    )
